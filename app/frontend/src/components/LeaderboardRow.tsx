...
export function LeaderboardRow({ product, onAnalyse }: LeaderboardRowProps) {
  return (
    <div className="group flex items-center gap-4 rounded-xl border border-[#1e1e2e] bg-[#12121a] p-3 sm:p-4 hover:border-emerald-500/30 hover:bg-[#161620] transition-all duration-200 cursor-pointer">
      {/* Rank */}
      ...
      <div className="flex-1 min-w-0">
        ...
        <span className={`inline-block mt-1 rounded-full border px-2 py-0.5 text-[10px] font-medium ${getNicheColor(product.niche)}`}>
          {product.niche}
        </span>
      </div>
      ...
      <div className="hidden md:flex items-center gap-4">
        <div className="text-right">
          <p className="text-[10px] text-gray-500 uppercase tracking-wide">Achat</p>
          <p className="text-sm font-semibold text-gray-400">{product.buyPrice.toFixed(2)}€</p>
        </div>
        <div className="text-right">
          <p className="text-[10px] text-gray-500 uppercase tracking-wide">Vente</p>
